...
# Use envsubst to substitute CSP, BACKEND_PROXY_URL, and BACKEND_PUBLIC_URL variables in nginx template
envsubst '${CSP} ${BACKEND_PROXY_URL} ${BACKEND_PUBLIC_URL}' < /nginx.tmpl.conf > /etc/nginx/nginx.conf
