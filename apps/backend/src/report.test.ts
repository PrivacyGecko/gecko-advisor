...
describe('GET /api/report/:slug includes dataSharing meta', () => {
  let scanId = '';
  let slug = 'test-report-meta';
  const canRun = process.env.RUN_DB_TESTS === '1';

  beforeAll(async () => {
    if (!canRun) return;
    const scan = await prisma.scan.create({
      data: { input: 'https://meta.test', targetType: 'url', status: 'done', reportSlug: slug, score: 65, label: 'Caution' },
    });
    scanId = scan.id;
    await prisma.evidence.createMany({
      data: [
        { scanId, type: 'tracker', severity: 3, title: 'Tracker A', details: { domain: 'trk.example' } },
        { scanId, type: 'thirdparty', severity: 2, title: 'CDN', details: { domain: 'cdn.example' } },
        { scanId, type: 'cookie', severity: 2, title: 'Missing SameSite', details: {} },
      ],
    });
  });
